...
public class Polygon extends AbstractFigure {
   ...
   * 
   * @param x
   *          the x-coordinate of the upper left bound corner of the polygon
   * @param y
   *          the y-coordinate of the upper left bound corner of the polygon
   * @param corners
   *          number of corners
   * @param radius
  ...
  /**
   * Sets the amount of corners
   * @param corners new amount of corners (>= 3)
   * @throws IllegalArgumentException if less than 3 corners
   */
  public void setCorners(int corners) throws IllegalArgumentException {
    if (corners > 2) {
   ...
   * @param radius
   *          Radius of polygon (half of width/height)
   * @param xoffset
   *          Offset of X-coordinate
   * @param yoffset
   *          Offset of Y-coordinate
   */
  private void setPolygon(int corners, int radius, int xoffset, int yoffset)
      throws IllegalArgumentException {
    if (corners < 3 && radius < 0) {
      throw new IllegalArgumentException();
    }
    this.radius = radius;
    setCorners(corners);
  ...
  public boolean contains(int x, int y) {
    return (newPolygon != null && newPolygon.contains(x, y));
  }
  ...
  public Rectangle getBounds() {
    return new Rectangle(newPolygon.getBounds());
  }
